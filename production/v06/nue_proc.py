...
indir  = '/a/data/amsterdam/dl/dl_production_v00/larcv/mcc7_bnb_nueappearance_genie_to_larcv_fullreswire_inclusive_v00_%s' % part
outdir = 'nue_%s' % part
prefix = 'mcc7_bnb_nueappearance_genie_to_larcv_fullreswire_inclusive_v00_%s' % part
...
for flist in flist_v:
    ...
    if os.path.isfile(out_data):
        ...
        continue
    cwd=os.getcwd()
    cmd='cd %s;run_processor %s/hiresdiv_fullwire_mc_bnb_nue.cfg' % (outdir,cwd)
    for f in flist:
        cmd += ' %s' % f
    cmd += '; cd -'
